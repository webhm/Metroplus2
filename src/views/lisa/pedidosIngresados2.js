                    ...
                    m("div.d-flex.align-items-center.justify-content-between.mg-b-80.mg-t-10", [
                        m("h5.mg-b-0", "LISA:",
                            m("span.badge.badge-primary.tx-semibold.pd-l-10.pd-r-10.mg-l-5.tx-15",
                                PedidosIngresados.handleLabel(PedidosIngresados.idFiltro)
                            )
                        ),
                        m("div.d-flex.tx-14", [
                            m('.', { class: (PedidosIngresados.idFiltro === 1 ? 'd-none' : 'd-flex') }, [
                            ...
                            m("div.dropdown.dropleft", [
                                ...
                                m(".dropdown-menu.tx-13[aria-labelledby='dropdownMenuButton']", [
                                    ...
                                    m(m.route.Link, {
                                        class: 'dropdown-item',
                                        href: "/laboratorio/lisa/pedidos/ingresados/",
                                        params: { idFiltro: 2, fechaDesde: PedidosIngresados.fechaDesde, fechaHasta: PedidosIngresados.fechaHasta }
                                    ...
                                    m(m.route.Link, {
                                        class: 'dropdown-item',
                                        href: "/laboratorio/lisa/pedidos/ingresados/",
                                        params: { idFiltro: 3, fechaDesde: PedidosIngresados.fechaDesde, fechaHasta: PedidosIngresados.fechaHasta }
                                    }, [
                                    ...
                                    m(m.route.Link, {
                                        class: 'dropdown-item',
                                        href: "/laboratorio/lisa/pedidos/ingresados/",
                                        params: { idFiltro: 4, fechaDesde: PedidosIngresados.fechaDesde, fechaHasta: PedidosIngresados.fechaHasta }
                                    }, [
                                        "Pedidos de Consulta Externa"
                                    ]),
                                    m(m.route.Link, {
                                        class: 'dropdown-item',
                                        href: "/laboratorio/lisa/pedidos/ingresados/",
                                    ...
                                    m(m.route.Link, {
                                        class: 'dropdown-item',
                                        href: "/laboratorio/lisa/pedidos/ingresados/",
                                        params: { idFiltro: 7, fechaDesde: PedidosIngresados.fechaDesde, fechaHasta: PedidosIngresados.fechaHasta }
                                    ...
                                    m(m.route.Link, {
                                        class: 'dropdown-item',
                                        href: "/laboratorio/lisa/pedidos/ingresados/",
                                        params: { idFiltro: 9, fechaDesde: PedidosIngresados.fechaDesde, fechaHasta: PedidosIngresados.fechaHasta }
...
const PedidosIngresados = {
    ...
    fetchPedidosIngresados: () => {
        ...
        let url = "https://lisa.hospitalmetropolitano.org/v1/listar";

        if (PedidosIngresados.idFiltro == 1) {
            url += `?type=ingresadas&idFiltro=${PedidosIngresados.idFiltro}`;
        } else if (PedidosIngresados.idFiltro == 6) {
            url += `?type=pendienteMuestras&idFiltro=${PedidosIngresados.idFiltro}&fechaDesde=${PedidosIngresados.fechaDesde}&fechaHasta=${PedidosIngresados.fechaHasta}`;
        } else if (PedidosIngresados.idFiltro == 7) {
            url += `?type=reportes&idFiltro=${PedidosIngresados.idFiltro}&fechaDesde=${PedidosIngresados.fechaDesde}&fechaHasta=${PedidosIngresados.fechaHasta}`;
        } else {
            url += `?type=ingresadas&idFiltro=${PedidosIngresados.idFiltro}&fechaDesde=${PedidosIngresados.fechaDesde}&fechaHasta=${PedidosIngresados.fechaHasta}`;
        }
    ...
    oninit: (vnode) => {
        ...
        PedidosIngresados.idFiltro = parseInt(params.idFiltro) || 1;
    ...
    onupdate: (vnode) => {
        ...
        const newId = parseInt(params.idFiltro) || 1;
        ...
        if (newId !== PedidosIngresados.idFiltro ||
            (newId !== 1 && (newDesde !== PedidosIngresados.fechaDesde || newHasta !== PedidosIngresados.fechaHasta))) {
    ...
    loadPedidosReportes: () => {
        ...
        // Generamos el texto dinámico para el nombre del archivo y el título del reporte
        const tituloReporte = `Pedidos Ingresados (Del ${PedidosIngresados.fechaDesde} al ${PedidosIngresados.fechaHasta})`;
        const nombreArchivo = `Pedidos_Ingresados_${PedidosIngresados.fechaDesde}_al_${PedidosIngresados.fechaHasta}`;

        $.fn.dataTable.ext.errMode = "none";
        let table = $("#table-pedidosIngresados").DataTable({
            data: PedidosIngresados.pedidos,
            dom: '<"d-flex align-items-center justify-content-between mg-b-0"l<"html5buttons text-right"B>>rtip',
            responsive: true,
            buttons: [{
                    extend: "excel",
                    className: "badge badge-success mg-r-5 pd-l-10 pd-r-10 mg-l-5 tx-15",
                    text: '<i class="fas fa-file-excel mg-r-5"></i> Excel',
                ...
                {
                    extend: "pdf",
                    className: "badge badge-danger pd-l-10 pd-r-10 mg-l-5 tx-15",
                    text: '<i class="fas fa-file-pdf mg-r-5"></i> PDF',
    ...
    handleLabel(idFiltro) {
        if (idFiltro == 1) {
            return "Pedidos de Hoy";
        }
        if (idFiltro == 2) {
            return "Pedidos de Emergencia";
        }
        if (idFiltro == 3) {
            return "Pedidos de Hospitalización";
        }
        if (idFiltro == 4) {
            return "Pedidos de Consulta Externa";
        }
        if (idFiltro == 5) {
            return "Pedidos de Bco. de Sangre";
        }
        if (idFiltro == 6) {
            return "Muestras Pendientes";
        }
        if (idFiltro == 7) {
            return "Reportes";
        }
        if (idFiltro == 8) {
            return "Pedidos de Lab. a Domicilio";
        }
        if (idFiltro == 9) {
            ...
            return [
                ...
                m("div.content.content-components", m("div.container.mg-l-0.mg-r-0", { style: { "max-width": "100%" } }, [
                    m("ol.breadcrumb.df-breadcrumbs.mg-b-10", [
                        ...
                        m("li.breadcrumb-item", m(m.route.Link, { href: "/laboratorio" }, " Laboratorio ")),
