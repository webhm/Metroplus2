...
const listadoCortes = {
    ...
    view: (vnode) => {
        return m("table.table.listado-cortes", {style: {"width": "98%", "margin-left": "1%"}}, [
            m("tr", [
                m("th.tx-12", {style: { "width": "15%" }}, "INGRESAR CORTES:"),
                m("th.tx-12", {style: { "width": "15%" }}), 
                m("th.tx-12", {style: { "width": "50%" }}), 
			...
			corteModelo.listado.map(function(corte) {
                return [
                    m("tr#" +  corte.id, [
                        ...
                        m("td.tx-12.cortedescription", {style: {'width': '50%'}}, corte.descripcion),
                        m("td.tx-12", {style: {'width': '20%', 'padding': '0'}}, [   
                            m("button.btn.btn-xs.btn-primary.mg-l-2.tx-semibold[type='button'][corteid=" + corte.id + "]", {
                                onclick: function() {
                                    corteModelo.listado = corteModelo.listado.filter((item) => item.codigocorte !== this.getAttribute('corteid'));
                                },
